          ...
          tabs.map((t) => (
            <div key={t.id} className="flex items-center justify-between rounded-lg border border-border p-4">
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm text-muted">{t.song_name || "—"} {t.source_label && `· ${t.source_label}`}</p>
              </div>
              <button onClick={() => handleRemove(t.id)} className="ml-4 shrink-0 text-sm text-red-600 hover:text-red-800">
                Remove
              </button>
            </div>
          ))
